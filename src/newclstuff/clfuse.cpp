...
void exportOctree(int DEPTH, int*SIZE, unsigned** grid, short*leaf, std::string filename);
...
int main() {
	...
	try {
		...
		cl::Kernel leaf_clear(program, "leaf_clear");
		...
		cl::Kernel leaf_fuse(program, "leaf_fuse");
		...
		s = SIZE[DEPTH];
		short*host_leaf = new short[host_gridlen[DEPTH]*s*s*s*2];
		...
		exportOctree(DEPTH, SIZE, host_grid, host_leaf, "first.oct");
...
void exportOctree(int DEPTH, int*SIZE, unsigned** grid, short*leaf, std::string filename) {
	...
	while (q.size()) {
		...
		if (p.scale <= totscale) {
			...
			putc(clamp(leaf[p.ind*2]/256.+127.5, 0, 255), fp);
		}
	}
	fclose(fp);
}
