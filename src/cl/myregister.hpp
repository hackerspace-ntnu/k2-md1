...
struct MyRegistration {
	...
	int sw, sh;
	MyRegistration(int w, int h) {
		sw = w, sh = h;
		//reg = new Regitration(kinect.dev->getIrCameraParams(), kinect.dev->getColorCameraParams());
		float*lastx = new float[512];
		float*lasty = new float[512];
		float*thisx = new float[512];
		float*thisy = new float[512];
	}
	~MyRegistration() {
		//delete reg;
		delete lastx;
	...
	void convertDepth(float*zbuf, float*depth) {
		for (int j = 0; j < 640; j++) {
			for (int i = 0; i < 480; i++) {
